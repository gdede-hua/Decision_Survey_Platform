...
@Service
public class GenerateDataExcell {

	private ProblemsUserAhpRepository problemsUserAHPRepository;

	public GenerateDataExcell(ProblemsUserAhpRepository problemsUserAHPRepository) {
	...
	public void makeExceelPerUser(ProblemUserAhp problemUserAhp, Sheet sheet) {
		...
		Row headerBottom = sheet.getRow(rowCounter)!=null ? sheet.getRow(rowCounter) : sheet.createRow(rowCounter);
		...
		for ( Criteria criteria : criteriaList ) {
			if (criteria.getFactor().size()>1) {
				...
				FactorCrAhp factorCrAhp = problemUserAhp.getFactorCrAhp().stream()
						...
						.findAny().orElse(null);
				headerBottom.createCell(1).setCellValue(factorCrAhp.getCr());	
				rowCounter = rowCounter+3;
			}
		}
			...
			if (criteria.getFactor().size()==0) {
				...
				if (alternativesList.size()>0) {
					...
					AlternativesCriteriaCrAhp alternativesCriteriaCrAhp = problemUserAhp.getAlternativesCriteriaCrAhp().stream()
							...
							.findAny().orElse(null);
					headerBottom.createCell(1).setCellValue(alternativesCriteriaCrAhp.getCr());	
					rowCounter = rowCounter+3;
				}
			}
		}
		...
		for (Factor factor : factorList) {
			...
			if( alternativesList.size()>0) {
				...
				AlternativesFactorCrAhp alternativesFactorCrAhp = problemUserAhp.getAlternativesFactorCrAhp().stream()
						...
						.findAny().orElse(null);
				headerBottom.createCell(1).setCellValue(alternativesFactorCrAhp.getCr());	
				rowCounter = rowCounter+3;
			}
		}
